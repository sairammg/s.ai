...
# ============================
    ...
    with col_header2:
        st.markdown("""
        <div class="chat-header" style="position: relative; border: none; padding: 12px 0;">
            <div class="chat-logo">
                <span>💊</span>
                <span>S.AI</span>
            </div>
    ...
    with col_header3:
        if st.button("Salir", key="salir_header"):
            st.session_state.autenticado = False
            st.session_state.conversacion = []
            st.rerun()
    
    # Status bar si quedan pocas conversaciones
    if restantes <= 3:
        color = "#ef4444" if restantes <= 1 else "#f59e0b"
        st.markdown(f"""
        <div style="background: #f9fafb; border-bottom: 1px solid #e5e7eb; padding: 12px 24px; margin-bottom: 24px;">
            <div style="max-width: 720px; margin: 0 auto; text-align: center;">
                <span style="font-size: 13px; font-weight: 500; color: #6b7280;">
                    <span style="font-weight: 600; color: {color};">{restantes} / {limite}</span> conversaciones restantes hoy
                </span>
            </div>
        </div>
        """, unsafe_allow_html=True)
